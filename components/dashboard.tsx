...
import { ItemDialog } from "@/components/item-dialog" // Modal component for adding/editing items.
import { ITEM_TYPES, ItemType } from "@/lib/types" // Type definitions and constants for item types.
import { LogOut, Plus, Search, ChevronLeft, ChevronRight, Loader2 } from "lucide-react" // Icons for UI elements.
import { Badge } from "@/components/ui/badge" // Component for displaying category or filter tags.

/**
 * @function Dashboard
 * @description The main layout component for the application. It manages the display of
 * items, search and filtering controls, server-side pagination, and user authentication actions (logout).
 * All filtering and pagination is handled by the API, making it scalable for large datasets.
 * 
 * @returns {JSX.Element} The rendered dashboard interface.
 */
export function Dashboard() {
    ...
    // Destructure item data, filters, and state setters from the items context.
    // NOTE: 'items' now comes pre-filtered and pre-paginated from the API (not 'filteredItems').
    const {
        items,              // Array of items for the current page (already filtered and paginated by API).
        selectedType,       // Current filter value for item type.
        setSelectedType,    // Setter for the item type filter.
        selectedTheme,      // Current filter value for item theme.
        setSelectedTheme,   // Setter for the item theme filter.
        searchQuery,        // Current value for the search query.
        setSearchQuery,     // Setter for the search query.
        themes,             // Array of unique themes available across all items (loaded separately).
        showFavorites,      // Boolean state to filter by favorite items.
        setShowFavorites,   // Setter for the showFavorites state.
        pagination,         // Object containing pagination state: { page, totalPages, totalItems }.
        setPage,            // Setter to change the current page number (triggers API call).
            ...
            <div className="container mx-auto px-4 py-8">
                <div className="space-y-4 mb-8">
                    ...
                    <div className="flex flex-wrap gap-2">
                        ...
                        {/* Dynamic Item Type Filters (Books, Movies, etc.) */}
                        {ITEM_TYPES.map((type) => (
                            <Badge
                                key={type.value}
                                variant={selectedType === type.value ? "default" : "outline"}
                                className="cursor-pointer px-4 py-2"
                                onClick={() => setSelectedType(type.value as ItemType)} // Set the item type filter state.
                            >
                                {type.icon} {type.label}
                            </Badge>
                        ))}
                ...
                {/* Conditional Rendering based on loading state and items count */}
                {isLoading ? (
                    /* Loading State: Displayed while fetching data from the API */
                    <div className="flex items-center justify-center py-16">
                        <Loader2 className="w-8 h-8 animate-spin text-muted-foreground" />
                    </div>
                ) : items.length === 0 ? (
                    /* Empty State: Displayed when no items match the current filters/search */
                    <div className="text-center py-16">
                        <p className="text-muted-foreground text-lg mb-4">
                            Nenhum item encontrado
                        </p>
                    ...
                    <>
                        {/* Item Grid: Displays the items for the current page */}
                        {/* NOTE: No local slicing needed - 'items' is already paginated by the API */}
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {items.map((item) => (
                                <ItemCard key={item.id} item={item} />
                            ))}
                        ...
                        {pagination.totalPages > 1 && (
                            <div className="flex items-center justify-center gap-4 mt-8">
                                ...
                                <Button
                                    variant="outline"
                                    size="icon"
                                    disabled={pagination.page === 1 || isLoading} // Disabled on the first page or while loading.
                                    onClick={() => setPage(pagination.page - 1)} // Decrement page number (triggers API call).
                                >
                                    <ChevronLeft className="w-4 h-4" />
                                </Button>

                                {/* Page Indicator with total items count */}
                                <span className="text-sm text-muted-foreground">
                                ...
                                <Button
                                    variant="outline"
                                    size="icon"
                                    disabled={pagination.page === pagination.totalPages || isLoading} // Disabled on the last page or while loading.
                                    onClick={() => setPage(pagination.page + 1)} // Increment page number (triggers API call).
                                >
                                    <ChevronRight className="w-4 h-4" />
                                </Button>
                            </div>
